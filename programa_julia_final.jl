...
# s es para definir una nueva semilla y generar números aleatorios 
...
function base(N)
    """
    Función que genera la base de R^n.
    """
    M = [[0 for i in 1:N] for i in 1:N]
    for i in 1:N
        M[i][i]= 1
    end
    return M
...
function PI0(vectors, eigen_values, eigen_vectors, N, t)
    """
    Probabilidad de retorno usando álgebra lineal (matrices)
    """
    proyecciones = abs2.(dot.(vectors, eigen_vectors'))    
    exp_val = exp.(-im*(eigen_values .- eigen_values').*t)
    resultado = sum(proyecciones .* exp_val)
    return fetch(resultado/N)
end

######################################### PROBA DE RETORNO DEPENDIENTE DEL TIEMPO ##########################################

#Los vectores con los cuales se proyectan los eigenvectores son la base de R^n 
function PI0_t(T_0, T_final, incremento)
    ...
    t = T_0
    while t<= T_final
        p_0 = real(PI0(base(N), eigen_values, eigen_vectors, N, t))
        append!(tiempo,t)
        append!(probas,p_0)
        t+= incremento
    end
